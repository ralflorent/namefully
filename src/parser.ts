...
import { ArrayStringValidator, ArrayNameValidator, Validators } from './validator';
...
export class ArrayNameParser extends Parser<Name[]> {
  parse(options: Partial<Config>): FullName {
    ...
    const fullName = new FullName(options);
    for (const name of this.raw) {
      if (name.isPrefix) {
        ...
        fullName.setLastName(new LastName(name.value, mother, fullName.config.surname));
      }
    }
    return fullName;
  }
